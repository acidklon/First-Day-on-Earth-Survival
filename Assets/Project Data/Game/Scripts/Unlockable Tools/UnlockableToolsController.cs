...
namespace Watermelon
{
    public class UnlockableToolsController : MonoBehaviour
    {
        ...
        public static FloatingTextBaseBehaviour ShowMessage(UnlockableTool unlockableTool, Vector3 position, Quaternion rotation)
        {
            ...
            FloatingTextBaseBehaviour floatingText;
            if (LocalizationManager.CurrentLanguage == "English")
                floatingText = FloatingTextController.SpawnFloatingText(FLOATING_TEXT_HASH, FLOATING_TEXT_MESSAGE, position, rotation, instance.floatingTextColor);
            else if (LocalizationManager.CurrentLanguage == "Russian")
                floatingText = FloatingTextController.SpawnFloatingText(FLOATING_TEXT_HASH, "Необходимо!", position, rotation, instance.floatingTextColor);
            else
                floatingText = FloatingTextController.SpawnFloatingText(FLOATING_TEXT_HASH, FLOATING_TEXT_MESSAGE, position, rotation, instance.floatingTextColor);
