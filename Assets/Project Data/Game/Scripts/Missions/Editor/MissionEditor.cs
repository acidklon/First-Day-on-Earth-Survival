...
namespace Watermelon
{
    public class MissionEditor : EditorWindow
    {
        ...
        private void OnEnable()
        {
            ...
            SetUpReordableList();
            currentSideBarWidth = SavesYG.GetInt(PREFS_WIDTH, SIDEBAR_WIDTH);
        ...
        private void OpenLastActiveMission()
        {
            ...
            {
                if (missions.Count > 0 && SavesYG.HasKey(PREFS_MISSION))
                {
                    int missionIndex = Mathf.Clamp(SavesYG.GetInt(PREFS_MISSION, 0), 0, missions.Count - 1);
        ...
        {
            SavesYG.SetInt(PREFS_MISSION, missionsReordableList.SelectedIndex);
            //PlayerPrefs.Save();
        ...
        private void HandleChangingSideBar()
        {
            ...
            if (separatorIsDragged)
            {
                if (Event.current.type == EventType.MouseUp)
                {
                    ...
                    SavesYG.SetInt(PREFS_WIDTH, currentSideBarWidth);
                    //PlayerPrefs.Save();
                    YandexGame.savesData.Save();
                    Event.current.Use();
                }
