...
namespace Watermelon
{
    public class MissionUIPanel : MonoBehaviour
    {
        ...
        public void Initialise()
        {
            ...
            spawn.transform.SetParent(UIController.GamePage.transform);
            floatingCloundSpawn = spawn.AddComponent<RectTransform>();

            GameObject target = new GameObject("[floating cloud target]");
            target.transform.SetParent(UIController.GamePage.transform);
            ...
            if (activeMission != null)
            {
                ...
                else
                {
                    if (LocalizationManager.CurrentLanguage == "English")
                        ...
                        rewardText.text = activeMission.RewardType == MissionRewardType.Tool ? "ИНСТРУМЕНТ РАЗБЛОКИРОВАН" : "НОВАЯ ПОСТРОЙКА";
                }
            }
        }
